...
class Experiment:
    ...
    def run_algorithm(self, algorithm):
        ...
        for _ in range(self.num_runs):
            if algorithm == 'PSO':
                pso = PSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w=self.w, c1=self.c1, c2=self.c2)
                best_solution, best_fitness, convergence_data, runtime = pso.optimize()
                data.append({'best_solution': best_solution, 'best_fitness': best_fitness, 'convergence_data': convergence_data, 'runtime': runtime})
            elif algorithm == 'APSO':
                apso = APSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w_min=self.w_min, w_max=self.w_max, c1=self.c1, c2=self.c2)
                best_solution, best_fitness, convergence_data, runtime = apso.optimize()
                data.append({'best_solution': best_solution, 'best_fitness': best_fitness, 'convergence_data': convergence_data, 'runtime': runtime})
            elif algorithm == 'HPSO':
                hpso = DiscretePSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w=self.w, c1=self.c1, c2=self.c2)
                ...
                stochastic_hill_climber = StochasticHillClimber(self.tsp_instance, max_iterations=self.max_iterations, max_stagnation=self.max_stagnation)
                best_solution, best_fitness, convergence_data, runtime = stochastic_hill_climber.optimize()
                data.append({'best_solution': best_solution, 'best_fitness': best_fitness, 'convergence_data': convergence_data, 'runtime': runtime})
            elif algorithm == 'spso':
                spso = SpatialPSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w=self.w, c1=self.c1, c2=self.c2, neighborhood_size=self.neighborhood_size)
                best_solution, best_fitness, convergence_data, runtime = spso.optimize()
                data.append({'best_solution': best_solution, 'best_fitness': best_fitness, 'convergence_data': convergence_data, 'runtime': runtime})
            elif algorithm == 'depso':
                deps = DEPSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w=self.w, c1=self.c1, c2=self.c2, cr=self.cr, f=self.f)
                best_solution, best_fitness, convergence_data, runtime = deps.optimize()
                data.append({'best_solution': best_solution, 'best_fitness': best_fitness, 'convergence_data': convergence_data, 'runtime': runtime})
            elif algorithm == 'pppso':
                pppso = PredatorPreyPSO(self.tsp_instance, population_size=self.population_size, max_iterations=self.max_iterations, w=self.w, c1=self.c1, c2=self.c2, fear_factor=self.fear_factor)
    ...
    def run_population_size_experiments(self, population_sizes):
        results = Parallel(n_jobs=-1, backend='multiprocessing')(
            delayed(self.run_population_size_experiment)(pop_size) for pop_size in population_sizes)
    ...
    def run_inertia_weight_experiments(self, inertia_weights):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_inertia_weight_experiment, [(inertia_weight,) for inertia_weight in inertia_weights])
    ...
    def run_acceleration_coefficients_experiments(self, coefficient_combinations):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_acceleration_coefficients_experiment, [(c1, c2) for c1, c2 in coefficient_combinations])
    ...
    def run_neighborhood_size_experiments(self, neighborhood_sizes):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_neighborhood_size_experiment, [(neighborhood_size,) for neighborhood_size in neighborhood_sizes])
    ...
    def run_cr_f_experiments(self, cr_f_combinations):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_cr_f_experiment, [(cr, f) for cr, f in cr_f_combinations])
    ...
    def run_fear_factor_experiments(self, fear_factors):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_fear_factor_experiment, [(fear_factor,) for fear_factor in fear_factors])
    ...
    def run_w_min_w_max_experiments(self, w_min_w_max_combinations):
        with multiprocessing.get_context('spawn').Pool() as pool:
            results = pool.starmap(self.run_w_min_w_max_experiment, [(w_min, w_max) for w_min, w_max in w_min_w_max_combinations])
